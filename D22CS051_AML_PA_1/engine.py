...
def log_image_table(images, predicted, labels, probs):
    ...
    for img, pred, targ, prob in zip(images.to("cpu"), predicted.to("cpu"), labels.to("cpu"), probs.to("cpu")):
        table.add_data(wandb.Image(img[0].numpy()*255), pred, targ, *prob.numpy()[:10])
...
def training_step(
    ...
    if profiler != None:
      ...
      with tqdm(dataloader, unit="batch",leave=False) as tepoch:
        for step,(X, y) in enumerate(tepoch):  # loop in batches
          if step >= (1 + 1 + 3) * 2:
            break
          tepoch.set_description(f"training batch: {step}")
...
def testing_step(
    ...
    with torch.inference_mode(): # disabling inference mode for aqcuiring gradients of perturbed data
      with tqdm(dataloader, unit="batch",leave=False) as tepoch:
        for step,(X, y) in enumerate(tepoch):  # loop in batches
          tepoch.set_description(f"testing batch: {step}")
          X, y = X.to(device), y.to(device)  # sending the data to target device
          # print(f"shape of X: {X.shape}, shape of y: {y.shape}")
...
def train(
    ...
    Args:
      ...
      scheduler (torch.optim.lr_scheduler): Learning Rate scheduler. Defaults to None.
    ...
    if profiler == None:
      torch.save(best_model_params, f"models/{model.__class__.__name__}_epoch_{epochs}_optim_adam_"+save_info+".pth")
